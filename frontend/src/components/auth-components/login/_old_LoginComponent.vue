<template>
  <div class="container">
      <div class="logo">
        <router-link to="/"><img src="@/assets/logo-djob-verde.svg" alt=""></router-link>
      </div>
      <h1>SUA CONTA PARA TUDO DA D.JOB</h1>
        <form
          v-on:submit.prevent="loginSubmitUserForm()"
        >
          <!--INÍCIO BLOCO: E-mail-->
          <div class="form-group">
            <label for="email">
              <input
                type="text"
                id="email"
                name="email"
                placeholder="E-mail:"
                v-model="loginForm.username"
                :class="{
                  'is-invalid': isSubmitted && $v.loginForm.username.$error,
                }"
              />
            </label>
            <div
              v-if="isSubmitted && !$v.loginForm.username.required"
              class="invalid-feedback"
            >
              O campo e-mail é obrigatório!
            </div>
          </div>
          <!--FIM BLOCO: E-mail-->

          <!--INÍCIO BLOCO: Password-->
          <div class="form-group">
            <label for="password">
              <input
                type="password"
                id="password"
                name="password"
                class=""
                placeholder="Senha:"
                v-model="loginForm.password"
                :class="{
                  'is-invalid': isSubmitted && $v.loginForm.password.$error,
                }"
              />
            </label>
            <div
              v-if="isSubmitted && !$v.loginForm.password.required"
              class="invalid-feedback"
            >
              O campo password é obrigatório!
            </div>
          </div>
            <p>
              Ao criar uma conta, você concorda com a
              <router-link to="/politica-privacidade">
              Política de privacidade
              </router-link> e com os
              <router-link to="/termo-uso"> Termos de uso</router-link>
              da djob.com.br.
            </p>
            <button
              @click="submitLoginUser()"
              class=""
            >
              Entrar
            </button>
            <!--FIM BLOCO: Password-->
            <p class="center">
              Não tem uma conta cadastrada?<router-link to="/register">
                Junte-se a nós</router-link
              >
            </p>
            <!--INÍCIO BLOCO: Botão-->
          <!--FIM BLOCO: Botão-->
        </form>
    </div>
</template>

<script src="./Login.js"></script>

<style scoped>
  .container{
    display: grid;
    padding: 50px;
    justify-content: center;
    align-items: center;
    text-align: center;
  }

  form{
    max-width: 400px;
  }
  .logo{
    text-align: center;
    width: 200px;
    justify-self: center;
    margin: 20px;
  }

  .inscreva-se{
    display: flex;
    align-items: center;
  }


  p{
    font-size: 14px;
    margin: 20px;
  }

  a{
    text-decoration: underline;
    font-weight: bold;
    color: #666;
    transition: all 0.5s;
  }

   a:hover{
    text-decoration: underline;
    font-weight: bold;
    color: #6DADB7;
    transition: all 0.5s;
  }

  .invalid-feedback{
    color: red;
    font-size: 12px;
  }
</style>
